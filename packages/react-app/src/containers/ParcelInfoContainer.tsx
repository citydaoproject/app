...
function ParcelInfoContainer(): JSX.Element {
  ...
  if (loading) return <p className="parcel-info text-green-1">Sensor Data Loading ...</p>;
  if (error) console.log(error);
  if (error) return <p className="parcel-info text-red-600">Sensor Data Load Failed.</p>;
  console.log(data?.pebble_device_record[0]);
  return (
    <div className="parcel-info">
      <ParcelInfo sensorData={data?.pebble_device_record[0]}></ParcelInfo>
    </div>
  );
}
